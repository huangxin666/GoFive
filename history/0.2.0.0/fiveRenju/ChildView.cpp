...
BEGIN_MESSAGE_MAP(CChildView, CWnd)
	...
	ON_COMMAND(ID_CHANGESIDE, &CChildView::OnChangeside)
END_MESSAGE_MAP()
...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if(rcBroard.PtInRect(point)&&uGameState==GAME_STATE_RUN){
		...
		if(currentBoard->m_pFive[x][y].uState==0&&x<15&&y<15&&point.x>=2+BLANK&&point.y>=4+BLANK){
			...
			if(stepList.IsEmpty()){
				temp.step=1;
				searchRect=CRect(x-1,y-1,x+1,y+1);
			}else{
...
int CChildView::getStepScores(FIVEWND thisStep,FiveBoard* currentBoard)
{
	...
	stepScore=situationCount[0]*100000+situationCount[1]*10000+situationCount[2]*1000
		+situationCount[3]*1001+situationCount[4]*1000+situationCount[5]*400+situationCount[6]*350
...
{
	AISTEP AIstep=getBestStep(2,*currentBoard,-playerSide);
	STEP temp;
	stepList.GetAt(currentStep).current->isFlag=false;
	temp.step=stepList.GetAt(currentStep).step+1;
