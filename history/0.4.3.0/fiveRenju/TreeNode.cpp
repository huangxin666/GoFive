...
AISTEP TreeNode::searchBest2()
{
	...
	if (childs[sortList[planB].key]->currentScore >= 100000||
		(childs[sortList[planB].key]->currentScore >= 10000 && childs[sortList[planB].key]->getHighest(side) < 100000))
		result = AISTEP(childs[sortList[planB].key]->lastStep.uRow, childs[sortList[planB].key]->lastStep.uCol, 0);
	else if (sortList[bestPos].value<-50000||
		(sortList[bestPos].value < -9000 && childs[sortList[bestPos].key]->currentScore<900))
	{
		planB=getDefense();
	...
	else if (sortList[planB].value>-8000 && childs[sortList[planB].key]->currentScore > 790)
		result = AISTEP(childs[sortList[planB].key]->lastStep.uRow, childs[sortList[planB].key]->lastStep.uCol, 0);
	else
		result = AISTEP(childs[sortList[bestPos].key]->lastStep.uRow, childs[sortList[bestPos].key]->lastStep.uCol, 0);
...
int TreeNode::getDefense()
{
	...
	for (int i = 0; i < child_num; ++i)
	{
		if (childs[i]->getTotal(side) < min)
		{
			min = childs[i]->getTotal(side);
			flag = i;
		}
	}
	return flag;
...
void TreeNode::buildSortListInfo(int n, THREATINFO *threatInfo)
{
	...
	if (!hasSearch[i])
	{
		if (childs[i]->currentScore == 1000 || childs[i]->currentScore == 960)//减小无意义冲四的优先级
		{
			if (currentBoard->getPiece(childs[i]->lastStep.uRow, childs[i]->lastStep.uCol).getThreat(side)<900)
		...
		for (int n = 0; n < BOARD_COL_MAX; ++n)
		{
			if (currentBoard->getPiece(m, n).isHot() && currentBoard->getPiece(m, n).getState() == 0)
			{
				if (currentBoard->getPiece(m, n).getThreat(side) >= 100000)
				{
					...
					tempBoard.updateThreat(ban);
					tempNode = new TreeNode(tempBoard, high - 1, !isAI, score);//flag high-1
					addChild(tempNode);
					goto flag;
				}
				...
				{
					if (currentBoard->getPiece(m, n).getThreat(-side)>800 && currentBoard->getPiece(m, n).getThreat(-side) < 1100)
					{
						tempBoard = *currentBoard;
						score = currentBoard->getPiece(m, n).getThreat(-side);
						tempBoard.doNextStep(m, n, -side);
