...
AISTEP CChildView::getBestStepAI1(ChessBoard currentBoard, int state)
{
	ChessBoard tempBoard;
	AISTEP stepCurrent;
	AISTEP randomStep[225];
...
AISTEP CChildView::getBestStepAI2(ChessBoard currentBoard,int state)
{
	ChessBoard tempBoard;
	AISTEP stepCurrent;
	AISTEP randomStep[225];
...
AISTEP CChildView::getBestStepAI3(ChessBoard currentBoard, int state)
{
	ChessBoard tempBoard;
	AISTEP stepCurrent;
	AISTEP randomStep[225];
	...
	for (int i = 0; i<BOARD_ROW_MAX; ++i){
		for (int j = 0; j<BOARD_COL_MAX; ++j){
			...
			if (currentBoard.getPiece(i, j).getState() == 0){
				...
				StepScore = tempBoard.getStepScores(tempBoard.getPiece());
				//score = currentBoard.getGlobalScore(state);
				THREATINFO info = tempBoard.getThreatInfo(-state, 2);
				totalScore = info.totalScore;
				HighestScoreTemp = info.HighestScore;
				if (StepScore >= 100000){
					stepCurrent.score = StepScore;
