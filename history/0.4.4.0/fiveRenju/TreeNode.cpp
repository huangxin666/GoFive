...
void TreeNode::buildChildrenInfo(int *childrenInfo,int i)
{
	...
	{
		childrenInfo[i] = childs[i]->currentScore - childs[i]->getTotal(lastStep.getColor());
	...
	else
	{
		...
		if (temp.totalScore > childs[i]->getTotal(lastStep.getColor()) && childs[i]->getHighest(lastStep.getColor()) - temp.HighestScore < childs[i]->getHighest(lastStep.getColor()) / 2)
		{
...
AISTEP TreeNode::searchBest2()
{
	...
	if (needSearch)
	{
		...
		while (true)
		{
			...
			while (!isAllOut(childs.size() - searchNum, childs.size()))
			{
				Sleep(100);
			}
		...
		(threatInfo[sortList[bestPos].key].HighestScore >= 10000 && childs[sortList[bestPos].key]->currentScore <= 1200 ))
	{
		...
		else if (currentBoard->getPiece(childs[planB]->lastStep.uRow, childs[planB]->lastStep.uCol).getThreat(lastStep.getColor()) > 2000)
...
void TreeNode::buildSortListInfo(int n, THREATINFO *threatInfo, bool *hasSearch)
{
	...
	if (!hasSearch[i])
	{
		sortList[n].value = childs[i]->currentScore - childs[i]->getTotal(lastStep.getColor());
