    ...
    for (; chess_mode_len < 16; ++chess_mode_len, hash_size *= 2)
    {
        ...
        for (int index = 0; index < hash_size; ++index)
        {
            ...
            for (int l = 0; l < chess_mode_len; ++l)
            {
                ...
                chessModeHashTable[chess_mode_len][index*chess_mode_len + l] = normalType2HashType(type);
            }
        }
    }
...
CHESSMODE_BASE ChessBoard::normalType2HashType(int chessModeType)
{
    if (chessModeType == TRIE_6_CONTINUE)
    {
        return MODE_BASE_6_b;
    }
